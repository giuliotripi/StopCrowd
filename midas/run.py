...
def run(input_path, model_path=os.path.join("midas","model-f6b98070.pt"), model_type="large", optimize=True):
    ...
    out = utils.write_depth(prediction, bits=2)
    return prediction, out

    print("finished")

def run_2(input_path, model_path=os.path.join("midas","model-f6b98070.pt"), model_type="large", optimize=True):
    """Run MonoDepthNN to compute depth maps.
    ...
    with torch.no_grad():
        ...
        # output depth
        output_path = os.path.join("predictions", "midas_depth", os.path.basename(os.path.normpath(os.path.splitext(input_path)[0])))
        
        out = utils.write_depth(path=output_path, depth=prediction, bits=1)
